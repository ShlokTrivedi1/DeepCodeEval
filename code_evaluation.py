...
class LiteLLMEvaluator(DeepEvalBaseLLM):
    """Custom DeepEval LLM wrapper to route evaluation through LiteLLM"""
...
def evaluate_code(generated_code: str, reference_code: str = None, model_id: str = None, api_key: str = None):
    try:
        # Create custom evaluator if credentials provided
        custom_llm = None
        if model_id and api_key:
            custom_llm = LiteLLMEvaluator(model_id=model_id, api_key=api_key)
